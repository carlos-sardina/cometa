...
import { AccordionItem, Background, Card, FlexContainer, StyledButton, StyledCometaTypography } from './styles';
          ...
          <Grid item xs={12}>
            <Card>
              ...
              <FlexContainer>
                ...
                <StyledCometaTypography>{total === 0 ? '--' : FormatNumber(total + '')}</StyledCometaTypography>
              </FlexContainer>
            </Card>
          </Grid>
          {
              ...
              <AccordionDetails>
                {
                  due.map(item => (
                    <AccordionItem key={item.id}>
                      ...
                      <Checkbox {...label} onChange={({ target: { checked } }) => setTotal((total) => total = (checked ? total + Number(item.price) + Number(item.interest) : total - Number(item.price) - Number(item.interest)))} />
                    </AccordionItem>
                ))}
              </AccordionDetails>
            </Accordion>
              ...
              <AccordionDetails>
                {
                  outstanding.map(item => (
                    <AccordionItem key={item.id}>
                      ...
                      <Checkbox {...label} onChange={({ target: { checked } }) => setTotal((total) => total = (checked ? total + Number(item.price) : total - Number(item.price)))} />
                    </AccordionItem>
                ))}
              </AccordionDetails>
            </Accordion>
